...
int getMinMaxNaif(int *array, unsigned arraySize, unsigned& minPos, unsigned& maxPos, int& minVal, int& maxVal)
{
    minPos = 0;
    maxPos = 0;
    minVal = *array;
    ...
    if(minPos!=0&&maxPos!=0)//if both doesnt equal 0 -> problem above
    {
        int val_return = 0;
        if(minPos<maxPos)
        {
...
int getMinMaxMoinsNaif(int *array, unsigned arraySize, unsigned& minPos, unsigned& maxPos, int& minVal, int& maxVal)
{
    minPos = 0;
    maxPos = 0;
    minVal = *array;
    ...
    for(unsigned ite=0;i<arraySize;ite++)
    {
        if(minVal>*(array + ite))//Looking for the minimum
        {
            minVal = *(array + ite);
